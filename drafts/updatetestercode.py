...
if st.button("GET STARTED"):
    ...
    message = "Please rearrange the priorities by typing the numbers in sequence."
...
if "industryConfirmed" in st.session_state and st.button("Confirm the Role List"):
    ...
    st.write("Final Consolidated Guest List:")
    st.dataframe(final_list_df)
     
     # Save the DataFrame to an Excel file in memory
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        final_list_df.to_excel(writer, index=False, sheet_name='Sheet1')
        writer.save()
        processed_data = output.getvalue()  
    # Create a download button and offer the Excel file for download
    st.download_button(
        label="Download Guest List as Excel",
        data=processed_data,
        file_name="guest_list.xlsx",
