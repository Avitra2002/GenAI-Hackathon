...
with st.sidebar:
    sys_message = "Input your event requirements below!"
...
def allocate_invites(num_invites, top_designations, user_preferred_designations):
    ...
    # Identify the top 10 industries based on their rank
    top_industries = st.session_state.indus[:10]
    # Filter the DataFrame to include only individuals within the top 10 industries and top designations
    filtered_df = df[df['Industry'].isin(top_industries) & df['Designation'].isin(top_designations)]
    ...
    for designation in top_designations:
        if designation in user_preferred_designations:
            ...
            invites_allocation[designation] = min(base_allocation_per_designation + extra_invites, len(filtered_df[filtered_df['Designation'] == designation]))
        else:
            # Allocate base invites to other designations
            invites_allocation[designation] = min(base_allocation_per_designation, len(filtered_df[filtered_df['Designation'] == designation]))
    ...
    return invites_allocation, filtered_df
...
invites_allocation, filtered_df= allocate_invites(st.session_state.num_invites, top_designations, user_preferred_designations)
...
if "industryConfirmed" in st.session_state and st.button("SHOW THE RESULT ✨"):
    ...
    for designation, num_invites in invites_allocation.items():
        designation_filtered_df = filtered_df[filtered_df['Designation'].str.contains(designation, case=False)].head(num_invites)
        
        if not designation_filtered_df.empty:
            st.markdown(f"#### Top invites for {designation} within the Top Industries:")
            st.dataframe(designation_filtered_df)
            final_list_df = pd.concat([final_list_df, designation_filtered_df], ignore_index=True)
